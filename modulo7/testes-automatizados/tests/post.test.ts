...
describe("Testes para inserir posts na tabela labook_posts do banco de dados", () => {
  ...
  test("inserir um post na tabela", async () => {
    ...
    const idPost: string = await createPost(newPost);
    const result = await postDatabase.getPostById(idPost);

    expect(result).toBeDefined()
    expect(result[0].id).toEqual(idPost)
    afterAll(async () => {
      await TestPostRemove.remove(idPost);
    });
  });
});
